...
export default function JobTypeStep({ navigation }) {
  ...
  return (
    <View style={styles.flex}>
      <Header
        title="JobHunt" showLogo onBack={handleBack}
      ...
      <ScrollView style={styles.flex} contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <Text style={styles.heading}>What type of job are you looking for?</Text>
        <Text style={styles.sub}>Select the option that best matches your current search goals.</Text>
        {JOB_TYPE_OPTIONS.map((opt) => (
          <RadioCard
            key={opt.value} icon={opt.icon} label={opt.label} sub={opt.sub}
            selected={answers.jobType === opt.value}
            onPress={() => setAnswer("jobType", opt.value)}
          />
        ))}
        {!answers.jobType && <Text style={styles.hint}>Selection required to continue</Text>}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  hint: { fontSize: FONT_SIZE.sm, color: C.TEXT_MUTED, textAlign: "center", marginTop: SPACING.sm },
