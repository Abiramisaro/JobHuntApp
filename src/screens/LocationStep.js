...
import { WORKING_STYLES, CITY_SUGGESTIONS, STEP_IDS } from "../constants/flowConfig";
...
export default function LocationStep({ navigation }) {
  ...
  const addCity    = (city) => { if (!selectedCities.includes(city)) setAnswer("preferredCities", [...selectedCities, city]); setCityInput(""); };
  const removeCity = (city) => setAnswer("preferredCities", selectedCities.filter((c) => c !== city));
    ...
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      <ScrollView style={styles.flex} contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={{ marginTop: SPACING.lg }}>
          ...
          <StyledInput iconName="search-outline" placeholder="Search by city or country..."
            value={cityInput} onChangeText={setCityInput} />
          {selectedCities.length > 0 && (
            <View style={styles.chipRow}>
              {selectedCities.map((city) => (
                <TouchableOpacity key={city} style={styles.cityChip} onPress={() => removeCity(city)}>
          ...
          <View style={styles.suggestionsRow}>
            {CITY_SUGGESTIONS.filter((c) => !selectedCities.includes(c)).map((city) => (
              <TouchableOpacity key={city} style={styles.suggestionChip} onPress={() => addCity(city)}>
...
const styles = StyleSheet.create({
  ...
  sub: { fontSize: FONT_SIZE.base, color: C.TEXT_SECONDARY, marginBottom: SPACING.md, lineHeight: 22 },
  sectionTitle: { fontSize: FONT_SIZE.xs, fontWeight: FONT_WEIGHT.semibold, color: C.TEXT_MUTED, letterSpacing: 1, marginBottom: SPACING.sm, marginTop: SPACING.sm },
  chipRow: { flexDirection: "row", flexWrap: "wrap", gap: SPACING.sm, marginTop: SPACING.sm },
  cityChip: { backgroundColor: C.PRIMARY, borderRadius: RADIUS.full, paddingHorizontal: SPACING.md, paddingVertical: 6 },
  cityChipText: { fontSize: FONT_SIZE.sm, color: C.WHITE, fontWeight: FONT_WEIGHT.medium },
  suggestionsLabel: { fontSize: FONT_SIZE.xs, fontWeight: FONT_WEIGHT.semibold, color: C.TEXT_MUTED, letterSpacing: 1, marginTop: SPACING.md, marginBottom: SPACING.sm },
  suggestionsRow: { flexDirection: "row", flexWrap: "wrap", gap: SPACING.sm },
  suggestionChip: { borderRadius: RADIUS.full, borderWidth: 1, borderColor: C.BORDER, paddingHorizontal: SPACING.md, paddingVertical: 7, backgroundColor: C.WHITE },
  suggestionText: { fontSize: FONT_SIZE.sm, color: C.TEXT_SECONDARY },
